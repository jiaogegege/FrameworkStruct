...
class ThemeContainer: OriginContainer
{
    ...
    {
        if let current = udAccessor.readString(key: udCurrentThemeKey)
        {
        ...
        else
        {
            ...
            self.mutate(key: TCGetKey.currentTheme, value: CustomTheme.init(theme: curThemeModel!))
            //保存当前主题标识
            self.udAccessor.write(key: udCurrentThemeKey, value: sPinkThemeFileName)
        }
    }
    ...
    override func commit(key: AnyHashable, value: Any)
    {
        if let k = key as? TCGetKey
        {
            ...
            {
                udAccessor.write(key: udCurrentThemeKey, value: value)
            }
        }
    }
